...
int VA_swap(V_Array* a, int i, int j) {
	char* mobile;
    void* eswap = malloc(a->e_size);
    if(!eswap){
        return RES_OUT_OF_MEM;
    }
    if (i >= a->a_count || j >= a->a_count || i < 0 || j < 0) {
    	return RES_OUT_OF_BOUNDS;
    }
    mobile = (char*) a->array;
...
void VA_qsort(V_Array* a, funccmp cmp, int inicio, int fin){
    ...
    i=inicio+1; j=fin;
    do{
        while( (i<=j) && (cmp( (mobile+(a->e_size * i)) , pivote) < 0) ) i++;
        while( (i<=j) && (cmp( (mobile+(a->e_size * j)) , pivote) > 0) ) j--;
        if(i<j && cmp( (mobile+(a->e_size * i)) , (mobile+(a->e_size * j)) ) != 0 ) {
        	VA_swap(a, i, j);
        } else if (i<j) {
        	i++;
        }
